...
def _dt_get_config_from_file():
    with open(os.path.join(script_path, '..', 'config', 'dt.conf'), 'r', encoding='UTF-8') as config_file:
...
def _dt_get_json_from_file(filenmae):
    ...
    with open(filenmae, 'r', encoding='UTF-8') as json_file:
            ...
            if init_flag:
                write_fd.writelines(
                    (
                        (
                            '\t'
                            + dt_config['FILE']['ENUM_PREFIX']
                            + f'{enum_prefix}_{enum_suffix}'.upper()
                        )
                        + ' = 0,\n'
                    )
                )
...
def _dt_trans_property_json_to_h_file(write_fd, _proterty_data):
    ...
    for property_id in _proterty_data:
        if property_id[dt_config['JSON']['DEFINE']][dt_config['JSON']['TYPE']] == dt_config['JSON']['ENUM']:
            ...
            _dt_write_enum_to_file(
                write_fd,
                f"// define property {property_id[dt_config['JSON']['ID']]} enum",
                'PROPERTY_' + property_id[dt_config['JSON']['ID']],
                enum_prefix,
                enum_val,
            )

        elif property_id[dt_config['JSON']['DEFINE']][
            dt_config['JSON']['TYPE']
        ] in [dt_config['JSON']['INT'], dt_config['JSON']['FLOAT']]:
            ...
            for k, v in property_id[dt_config['JSON']['DEFINE']].items():
                if v != '' and k != dt_config['JSON']['TYPE'] and k != dt_config['JSON']['UNIT']:
                    _dt_write_macro_to_file(
                        write_fd,
                        'PROPERTY_'
                        + property_id[dt_config['JSON']['ID']]
                        + '_'
                        + k,
                        f'({v})',
                    )
...
def _dt_trans_event_json_to_h_file(write_fd, _event_data):
    ...
    for event in _event_data:
        ...
        _dt_write_enum_to_file(
            write_fd,
            f"// define param id for event {event.get(dt_config['JSON']['ID'])}",
        ...
        for param in event.get('params'):
            if param[dt_config['JSON']['DEFINE']][dt_config['JSON']['TYPE']] == dt_config['JSON']['ENUM']:
                ...
                _dt_write_enum_to_file(
                    write_fd,
                    f"// define enum for param {param[dt_config['JSON']['ID']]}",
                    'EVEMT_'
                    + event.get(dt_config['JSON']['ID'])
                    + '_'
                    + param[dt_config['JSON']['ID']],
                    enum_prefix,
                    enum_val,
                )

            elif param[dt_config['JSON']['DEFINE']][
                dt_config['JSON']['TYPE']
            ] in [dt_config['JSON']['INT'], dt_config['JSON']['FLOAT']]:
...
def _dt_trans_action_json_to_h_file(write_fd, _action_data):
    ...
    for action in _action_data:
        ...
        _dt_write_enum_to_file(
            write_fd,
            f"// define input id for action {action.get(dt_config['JSON']['ID'])}",
        ...
        _dt_write_enum_to_file(
            write_fd,
            f"// define output id for action {action.get(dt_config['JSON']['ID'])}",
            'ACTION_' + action.get(dt_config['JSON']['ID']) + '_OUTPUT_ID',
            _dt_get_enum_list_from_ids(
                action.get(dt_config['JSON']['OUTPUT'])
            ),
        )
        ...
        for input_id in action.get('input'):
            if input_id[dt_config['JSON']['DEFINE']][dt_config['JSON']['TYPE']] == dt_config['JSON']['ENUM']:
                ...
                _dt_write_enum_to_file(
                    write_fd,
                    f"// define enum for input id {input_id[dt_config['JSON']['ID']]}",
                    'ACTION_INPUT_'
                    + action.get(dt_config['JSON']['ID'])
                    + '_'
                    + input_id[dt_config['JSON']['ID']],
                    enum_prefix,
                    enum_val,
                )

            elif input_id[dt_config['JSON']['DEFINE']][
                dt_config['JSON']['TYPE']
            ] in [dt_config['JSON']['INT'], dt_config['JSON']['FLOAT']]:
        ...
        for output in action.get('output'):
            if output[dt_config['JSON']['DEFINE']][dt_config['JSON']['TYPE']] == dt_config['JSON']['ENUM']:
                ...
                _dt_write_enum_to_file(
                    write_fd,
                    f"// define enum for output id {output[dt_config['JSON']['ID']]}",
                    'ACTION_OUTPUT_'
                    + action.get(dt_config['JSON']['ID'])
                    + '_'
                    + output[dt_config['JSON']['ID']],
                    enum_prefix,
                    enum_val,
                )

            elif output[dt_config['JSON']['DEFINE']][
                dt_config['JSON']['TYPE']
            ] in [dt_config['JSON']['INT'], dt_config['JSON']['FLOAT']]:
                _dt_write_newline_to_file(write_fd,
...
def _dt_write_function_prototype(write_fd):
    with open(os.path.join(script_path, 'dt_fixed_content', 'dt_ble_prototype'), 'r', encoding='UTF-8') as file_fd:
        file_content = file_fd.read()
    _dt_write_newline_to_file(write_fd, file_content)


def _dt_generate_header_file(_json_data):
    with open(dt_config['FILE']['NAME_PREFIX'] + '.h', 'w', encoding='UTF-8') as h_file:
...
def _dt_get_property_function_name(property_id):
    return f'ble_property_{property_id.lower()}_get'


def _dt_set_property_function_name(property_id):
    return f'ble_property_{property_id.lower()}_set'
...
def _dt_trans_property_json_to_c_file(write_fd, _proterty_data):
    ...
    with open(os.path.join(script_path, 'dt_fixed_content', 'dt_ble_property'), 'r', encoding='UTF-8') as dt_property_file:
...
def _dt_trans_event_json_to_c_file(write_fd, _evnet_data):
    ...
    with open(os.path.join(script_path, 'dt_fixed_content', 'dt_ble_event'), 'r', encoding='UTF-8') as dt_event_file:
...
def _dt_trans_action_json_to_c_file(write_fd, _action_data):
    ...
    with open(os.path.join(script_path, 'dt_fixed_content', 'dt_ble_action'), 'r', encoding='UTF-8') as dt_action_file:
